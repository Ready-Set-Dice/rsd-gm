                ...
                <v-expand-transition>
                    ...
                    >
                        <template v-for="(item, i) in initiativeMenu">
                            <v-list-item
                                :key="'item-'+i"
                                v-if="!item.type && ((!isNPC && !item.npcOnly) || (!!isNPC))"
...
export default {
        ...
        return {
            ...
            initiativeMenu: [
                { 
                    ...
                    func: () => {
                        if (!!this.selected && !!this.selected.id) {
                            this.$rsd.dialog.open({name: 'memberAlertDialog', attrs: [false, 
                                {
                                    cid: this.selected.id,
                                    remove: 1,
                                    round: -1,
                                    title: '',
                                    description: '',
                ...
                { 
                    ...
                    func: () => {
                        if (!!this.selected && !!this.selected.id) {
                            this.$rsd.dialog.open({name: 'combatInitiativeDialog', attrs: [true, {...this.selected, id: this.selected.id}]})
                            this.close()
                        }
                    }
                },
                ...
                },
                {
                    ...
                    func: () => {
                        if (!!this.selected && !!this.selected.id) {
                            const nameTemplate = !!this.selected.name ? this.selected.name : this.$rsd.bestiary.getName(this.encmember.contextSelected.bid)
                            this.$rsd.dialog.open({name: 'npcNameDialog', attrs: [true, {...this.selected, name: nameTemplate}, {combat: true}]})
                            this.close()
                        }
                    },
                    npcOnly: true
                },
                {
                    ...
                    func: () => {
                        if (!!this.selected  && !!this.selected.id) {
                            this.$rsd.dialog.open({name: 'npcStyleDialog', attrs: [true, {...this.selected}, {combat: true}]})
                            this.close()
                        }
                    },
                },
                { 
                    ...
                    func: () => {
                        this.$rsd.dialog.open({name: 'confirmDialog', attrs:[
                            'Remove Combatant', 
                            `Are you sure you want to remove the member ${this.selected.name}?`,
                            () => {this.$rsd.combat.removeMember(this.selected)}
                        ]})
                        this.close()
                    },
                },
                ...
                { 
                    ...
                    icon: 'mdi-alarm-plus',
                    func: () => {
                        this.$rsd.dialog.open({name: 'roundAlertDialog', attrs: [false, 
                            {
                                remove: 1,
                                round: -1,
                                title: '',
    ...
    computed: {
        ...
        regularConditions() {
            return !!this.isNPC ? this.npcConditions.regular : this.playerConditions.regular
        },
        otherConditions() {
            return !!this.isNPC ? this.npcConditions.other : this.playerConditions.other
        },
        ...mapGetters({
    ...
    methods: {
        ...
        removeCombatant() {
            if (!!this.dyingQueue && this.dyingQueue.length > 0) {
                const selected = this.dyingQueue.shift()
                this.$rsd.combat.removeMember(selected)
            }
        },
        addDyingCombatant() {
            if (!!this.dyingQueue && this.dyingQueue.length > 0) {
                const selected = this.dyingQueue.shift()
                this.$rsd.combat.increaseMemberDying(selected.id, !!this.crit ? 2 : 1)
            }
        },
        openDieOrRemoveDialog(data) {
            if (!!data && !!data.valid && !!data.affected) {
                this.crit = data.crit
                this.$rsd.dialog.open({name: 'confirmDialog', attrs:[
                    'Dying or remove', 
                    `Do you wish to set '${data.affected.name}' to dying state, or remove them?`,
                    () => {this.addDyingCombatant()},
                    () => {this.removeCombatant()},
                    'Add Dying Condition',
                    'Remove Combatant'
                ]})
                this.dyingQueue.push({...data.affected})
            }
        },
        openDamageDialog(type) {
            if (!!type && !!this.selected && !!this.selected.id) {
                ...
                this.$rsd.dialog.open({name: 'damageDialog', attrs: [true, 
                    {value:1, affected: [this.selected], dmg: type}, 
        ...
        addRecentDamge(dmg) {
            if (!!dmg && !!dmg.type && !!dmg.value && dmg.value > 0 && !!this.selected && !!this.selected.id) {
                this.$rsd.combat.doDamage(this.selected._key, {dmg: dmg.type, value: dmg.value})
                this.close()
            }
        },
        openHealDialog() {
            if (!!this.selected && !!this.selected.id) {
                this.$rsd.dialog.open({name: 'healDialog', attrs: [true, {value:1, affected: [this.selected]}]})
                this.close()
            }
        },
        openTemporaryHPDialog() {
            if (!!this.selected && !!this.selected.id) {
                this.$rsd.dialog.open({name: 'temporaryHPDialog', attrs: [true, {value: 1, duration:0, affected: [this.selected]}]})
                this.close()
            }
        },
        openConditionDialog(type) {
            if (!!type && !!this.selected && !!this.selected.id) {
                ...
                if (!!duration && !!value) {
                    this.$rsd.dialog.open({name: 'addConditionBothDialog', attrs: [false, {type: type, duration:0, value:1, floor: false, reduce: false, affected: [this.selected]}, {immune: immune}]})
                } else if (!!duration) {
                    this.$rsd.dialog.open({name: 'addConditionDurationDialog', attrs: [false, {type: type, duration:0, affected: [this.selected]}, {immune: immune}]})
                } else if (!!value) {
                    this.$rsd.dialog.open({name: 'addConditionValueDialog', attrs: [false, {type: type, value:1, floor: false, reduce: false, affected: [this.selected]}, {immune: immune}]})
                } else {
                    this.$rsd.combat.addCondition(this.selected._key, {type: type})
                }
                this.close()
            }
        },
        addRecentCondition(cond) {
            if (!!this.selected && !!this.selected.id) {
                if (!!cond && !!cond.type) {
                    this.$rsd.combat.addCondition(this.selected._key, {...cond})
                }
                this.close()
            }
        },
        openPersistentDamageDialog(dmg) {
            if (!!dmg && !!this.selected && !!this.selected.id) {
                ...
                this.$rsd.dialog.open({name: 'addPersistentDamageDialog', attrs: [false, 
                    {type:'persistent damage', value: 1, dmg: dmg, affected: [this.selected]}, 
                    {immune: immune, resistant: resistant, vulnerable: vulnerable}]
                })
                this.close()
            }
        ...
        addRecentPersistentDamge(dmg) {
            if (!!dmg && !!dmg.type && !!dmg.value && dmg.value > 0 && !!this.selected && !!this.selected.id) {
                this.$rsd.combat.addCondition(this.selected._key, {type: 'persistent damage', dmg: dmg.type, value: dmg.value})
                this.close()
            }
        },
    ...
    created() {
        this.$rsd.combat.on('DieOrRemove', this.openDieOrRemoveDialog)
    },
    destroyed() {
        this.$rsd.combat.removeListener('DieOrRemove', this.openDieOrRemoveDialog)
    },
}
</script>
